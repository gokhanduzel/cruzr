...
const CarCard: React.FC<CarCardProps> = ({
  ...
  return (
    <div className="p-4 flex flex-col w-80">
      <div className="bg-indigo-50 rounded-lg overflow-hidden shadow-lg flex flex-col" style={{ minHeight: "400px" }}>
        {carData.images && carData.images.length > 1 ? (
          <div className="relative aspect-w-16 aspect-h-9">
            ...
            {carData.images.map((image, index) => (
              <img key={index} src={image} alt={`${carData.make} ${carData.carModel}`} className={`w-full object-cover duration-500 ease-in-out ${index === activeIndex ? 'block' : 'hidden'}`} style={{ height: '200px' }} />
            ))}
          </div>
        ) : (
          <img src={carData.images[0]} alt={`${carData.make} ${carData.carModel}`} className="aspect-w-16 aspect-h-9 object-cover object-center w-full" style={{ height: "200px" }} />
        )}
        ...
        {!isOwner && onChatStart && (
          <button
            ...
            className="p-2 text-white bg-blue-500 hover:bg-blue-700 rounded-b-lg text-sm"
          >
            Chat with Owner
          </button>
        )}
